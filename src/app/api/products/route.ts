...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build a stable cache key — skip caching for free-text search (unbounded combinations)
    const isPublic = activeOnly !== "false";
    const cacheKey = !search && isPublic
      ? `sarvaa:products:${page}:${limit}:${categoryQuery ?? ""}:${featured ?? ""}:${sortBy}:${minPrice}:${maxPrice}`
      : null;
    ...
    if (cacheKey) {
      await setCached(cacheKey, payload, CACHE_TTL.products);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    await invalidatePrefix("sarvaa:products");
