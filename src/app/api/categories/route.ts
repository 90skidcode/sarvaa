...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `sarvaa:categories:${activeOnly ?? "all"}:${page}:${limit}`;
    ...
    await setCached(cacheKey, payload, CACHE_TTL.categories);
...
export async function POST(request: NextRequest) {
  try {
    ...
    await invalidatePrefix("sarvaa:categories");
