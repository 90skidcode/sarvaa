      ...
      <div className="container mx-auto px-3 sm:px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-start gap-2 sm:gap-4 mb-4 sm:mb-8">
            <Link href="/profile/orders">
              <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 sm:h-10 sm:w-10 flex-shrink-0">
                <ArrowLeft className="h-4 sm:h-5 w-4 sm:w-5" />
              </Button>
            </Link>
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900 truncate">{order.orderNumber}</h1>
                <Badge className={`${statusColors[order.status]} border text-xs sm:text-sm px-2 py-1 whitespace-nowrap`}>
                ...
                >
                  <Download className="h-3 sm:h-4 w-3 sm:w-4 mr-1" /> <span className="hidden sm:inline">Download Invoice</span><span className="sm:hidden">Invoice</span>
                </Button>
              </div>
              <p className="text-gray-500 text-xs sm:text-sm mt-1">Order placed on {formatDate(order.createdAt)}</p>
            </div>
          </div>
                ...
                <div className="flex sm:flex-row flex-col gap-2 sm:justify-between relative z-10">
                  {(() => {
                    ...
                    return steps.map((step, idx) => {
                      ...
                      return (
                        <div key={step.id} className="flex sm:flex-col flex-row items-center sm:items-center gap-2 sm:gap-0">
                          <div className={`
                          ...
                          `}>
                            <step.icon className={`h-4 sm:h-5 w-4 sm:w-5 ${isCurrent ? 'animate-pulse' : ''}`} />
                          </div>
                          <div className="sm:mt-3 flex flex-col items-start sm:items-center">
                            ...
                            {logTime && (
                              <span className="text-[8px] sm:text-[10px] text-gray-500 whitespace-nowrap sm:mt-1 sm:text-center font-medium">
                                <span className="hidden sm:block">{new Date(logTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>
                                <span className="hidden sm:block">{new Date(logTime).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                                <span className="sm:hidden">{new Date(logTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })} • {new Date(logTime).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                              </span>
                            )}
                          </div>
                        </div>
            ...
            <Card className="md:col-span-2 border-none shadow-lg">
              ...
              <CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-6">
                {order.items.map((item) => (
                  <div key={item.productId} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-900 leading-tight mb-0.5 sm:mb-1 text-sm sm:text-base truncate">{item.product?.name}</p>
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="font-bold text-[#743181] text-sm sm:text-base">₹{(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
                ...
                <div className="border-t pt-3 sm:pt-4 mt-3 sm:mt-4">
                  ...
                  <div className="flex justify-between items-center mt-3 sm:mt-4 pt-3 sm:pt-4 border-t">
                    <span className="font-bold text-gray-900 text-sm sm:text-lg">Total</span>
                    <span className="font-bold text-[#743181] text-lg sm:text-2xl">₹{order.total.toFixed(2)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <Card className="border-none shadow-lg">
                ...
                <CardContent className="p-4 sm:p-6 pt-0">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 line-clamp-2">{order.address || 'Sarvaa Sweets Main'}</p>
                </CardContent>
              </Card>
              ...
              <Card className="border-none shadow-lg">
                ...
                <CardContent className="space-y-1.5 p-4 sm:p-6 pt-0">
                  <p className="font-semibold text-gray-900 text-sm">{order.name || 'Guest User'}</p>
                  {order.email && (
              ...
              <Card className="border-none shadow-lg">
                ...
                <CardContent className="p-4 sm:p-6 pt-0">
                  <p className="font-semibold text-gray-900 uppercase text-sm">{order.paymentMethod}</p>
                  <p className="text-xs sm:text-sm text-gray-500">Pay at pickup</p>
                </CardContent>
              </Card>
