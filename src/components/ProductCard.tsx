...
import { CheckCircle2, Heart, Loader2, ShoppingCart } from 'lucide-react'
...
export function ProductCard({
  ...
  const cartItem = cartItems.find(
    (i) => i.productId === id && i.variantType === variantData.type && i.variantValue === variantData.value
  )
  ...
  if (viewMode === 'list') {
    return (
      <Card className={`group overflow-hidden rounded-2xl bg-white shadow-sm hover:shadow-md transition-all duration-300 p-0 ${isInCart ? 'border-2 border-purple-400' : 'border border-gray-100'}`}>
          ...
          >
            ...
            {badge && (
              <div className="absolute top-3 left-3 z-10">
                <span className="bg-purple-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg uppercase tracking-wider">
                  {badge}
                </span>
              </div>
            )}
            ...
            <div className="mt-auto flex flex-col 2xl:flex-row 2xl:items-end justify-between gap-4 sm:gap-6 pt-4 border-t border-gray-50">
              ...
              <div className="flex items-end justify-between 2xl:justify-start gap-6 w-full 2xl:w-auto">
                ...
                <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                  {isInCart ? (
                    <div className="flex items-center bg-purple-50 border-2 border-purple-400 rounded-xl overflow-hidden h-11">
                      <button
                        onClick={() => {
                          if (cartItem!.quantity <= 1) removeItem(id, variantData.type, variantData.value)
                          else updateQuantity(id, variantData.type, variantData.value, cartItem!.quantity - 1)
                        }}
                        className="h-full w-10 flex items-center justify-center text-purple-600 hover:bg-purple-100 transition-colors font-bold text-lg"
                      >
                        −
                      </button>
                      <span className="w-10 text-center font-bold text-purple-700 text-base">
                        {cartItem!.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(id, variantData.type, variantData.value, Math.min(cartItem!.quantity + 1, stock))}
                        className="h-full w-10 flex items-center justify-center text-purple-600 hover:bg-purple-100 transition-colors font-bold text-lg"
                      >
                        +
                      </button>
                    ...
                    <>
                      ...
                      <Button
                        ...
                        disabled={stock === 0}
                        className="h-11 px-4 sm:px-6 rounded-xl bg-purple-600 hover:bg-purple-700 text-white font-semibold shadow-sm transition-all active:scale-95 whitespace-nowrap min-w-[120px]"
                      >
                        Add to Cart
                      </Button>
                    </>
  ...
  return (
    <Card className={`group overflow-hidden rounded-3xl bg-white shadow-md hover:shadow-xl transition-all duration-300 flex flex-col h-full p-0 gap-0 ${isInCart ? 'border-2 border-purple-400 ring-2 ring-purple-100' : 'border-none'}`}>
      <CardContent className="p-0 flex flex-col h-full">
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            ...
            {/* In Cart badge */}
            {isInCart && (
              <div className="absolute top-3 left-3 z-10 flex items-center gap-1 bg-purple-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-lg">
          ...
          <div className="mt-auto space-y-3">
            ...
            {isInCart ? (
              <div className="flex items-center gap-2">
                {/* Cart stepper */}
                <div className="flex items-center flex-1 bg-purple-50 border-2 border-purple-400 rounded-xl overflow-hidden h-12">
                  <button
                    onClick={() => {
                      if (cartItem!.quantity <= 1) removeItem(id, variantData.type, variantData.value)
                      else updateQuantity(id, variantData.type, variantData.value, cartItem!.quantity - 1)
                    }}
                    className="h-full w-11 flex items-center justify-center text-purple-600 hover:bg-purple-100 transition-colors font-bold text-lg"
                  >
                    −
                  </button>
                  <span className="flex-1 text-center font-bold text-purple-700 text-base">
                    {cartItem!.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(id, variantData.type, variantData.value, Math.min(cartItem!.quantity + 1, stock))}
                ...
                {/* Add more (new variant) */}
                <Button
                  onClick={handleAddToCart}
                  className="h-12 w-12 rounded-xl bg-purple-600 hover:bg-purple-700 text-white shadow-md flex-shrink-0 p-0"
